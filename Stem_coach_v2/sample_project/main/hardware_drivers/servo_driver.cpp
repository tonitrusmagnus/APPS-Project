...
void Servo::setAngle(int angle) {
    ...
    int duty = map(angle,0,SERVO_MAX_DEGREE,500,2400);//(SERVO_MIN_COUNT) + ((SERVO_MAX_COUNT - SERVO_MIN_COUNT) * (angle / (SERVO_MAX_DEGREE))); //(angle * (SERVO_MAX_PULSEWIDTH - SERVO_MIN_PULSEWIDTH) / SERVO_MAX_DEGREE) + SERVO_MIN_PULSEWIDTH;
