/**
 * @file mp3_driver.hpp
 * @brief Declaration of the MP3Driver class for controlling an MP3 player module using UART communication.
 */
...
// Volume
#define MP3_MAX_VOLUME 30       // Max volume of MP3 Player is 30

// Buffer indexes for reading
#define MP3_BUFFER_COMMAND      3
#define MP3_BUFFER_HIGH_DATA    5
#define MP3_BUFFER_LOW_DATA     6

// BAUD Rate
#define MP3_UART_BAUD 9600 // Default BAUD rate of module is 9600
/**
 * @class MP3Driver
 * @brief Class for controlling an MP3 player module using UART communication.
 */
class MP3Driver {
private:
    gpio_num_t TxPin; /**< GPIO pin connected to the TX pin of the MP3 player module. */
    gpio_num_t RxPin; /**< GPIO pin connected to the RX pin of the MP3 player module. */
    uart_port_t UartNum; /**< UART port used for communication. */
    int readTimeout; /**< Timeout for reading from UART. */
    bool feedbackEnabled = false; /**< Flag indicating whether feedback is enabled. */
    bool playing; /**< Flag indicating whether the MP3 player is currently playing. */

     /**
     * @brief Reads feedback from the MP3 player module.
     * @param command The command for which feedback is expected.
     * @return Feedback status (0 or 1).
     */
    int getFeedback(char command);
    /**
     * @brief Sends data to the MP3 player module.
     * @param command The command byte.
     * @param dataMSB The most significant byte of data.
     * @param dataLSB The least significant byte of data.
     */
